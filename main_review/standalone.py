"""Dependency-free self-hosted service for Sergeant.

The service preserves Sergeant's authority separation:

- repository access is contained to one configured workspace;
- review requests are forced through the hardened App Bridge contract;
- public network binding requires an authentication token;
- GitHub webhook intake is HMAC verified and never executes PR-controlled code;
- the existing Command Center is served from the same installed package;
- no GitHub write, repository write, patch, or merge authority is present.
"""
...
class StandaloneServiceError(ValueError):
    """Raised when standalone service configuration or input is unsafe."""


class StandaloneServiceBusy(RuntimeError):
    """Raised when a second mission attempts to overlap an active mission."""
...
def normalize_github_webhook(event: str, payload: Mapping[str, Any], delivery: str) -> dict[str, Any]:
    """Normalize supported GitHub webhook events without trusting PR instructions."""
...
def _resource_roots() -> list[Path]:
    ...
    candidates.append(Path(__file__).resolve().parents[1] / "resources")
    candidates.append(Path(sysconfig.get_path("data")) / "share" / "sergeant" / "resources")
    output: list[Path] = []
...
def build_command_center_document() -> str:
    """Build the existing Command Center with the standalone browser bridge."""
    ...
    script = _read_resource("sergeant-command-center-v2.js")
    ...
    if "SERGEANT_CSS" in document or "SERGEANT_JS" in document or "SERGEANT_HOST_BOOTSTRAP" in document:
...
def _verdict_from_payload(payload: Mapping[str, Any]) -> str:
    for key in ("action", "verdict", "status"):
        value = payload.get(key)
        if isinstance(value, str) and value:
            return value.upper().replace("_", " ")
    nested = payload.get("verdict")
    if isinstance(nested, Mapping):
        value = nested.get("verdict")
        if isinstance(value, str) and value:
            return value
...
def _findings_from_payload(payload: Mapping[str, Any]) -> list[dict[str, Any]]:
    candidates = payload.get("top_findings")
    if isinstance(candidates, list):
        return [item for item in candidates if isinstance(item, dict)][:20]
...
class StandaloneApplication:
    """Transport-independent application layer used by HTTP and tests."""
    ...
    def execute_mission(self, supplied: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if action == "v2Mission":
            ...
            changed_files = supplied.get("changed_files") if isinstance(supplied.get("changed_files"), list) else []
            normalized_files = normalize_changed_files(self.settings.workspace_root, changed_files)
            request = {
                "root": str(self.settings.workspace_root),
                "mode": "changed_files" if normalized_files else "repository",
    ...
    def _run_locked(self, title: str, operation: Callable[[], dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            ...
            mission = {
                ...
                "summary": {"verdict": _verdict_from_payload(payload)},
            ...
            return {"schema_version": SERVICE_CONTRACT, "mission": mission, "state": self.runtime.state_snapshot()}
        finally:
...
def _handler_type(application: StandaloneApplication) -> type[BaseHTTPRequestHandler]:
    class Handler(BaseHTTPRequestHandler):
        ...
        def log_message(self, format_string: str, *args: object) -> None:
            safe = redact_secrets(format_string % args)
            print(f"[{_utc_now()}] {self.client_address[0]} {safe}", flush=True)
        ...
        def _allowed_origin(self) -> str | None:
            ...
            same_origin = f"http://{host}".rstrip("/")
            if origin == same_origin or origin in self.app.settings.allowed_origins:
                return origin
            return ""
        ...
        def _common_start(self) -> bool:
            ...
            if not self.app.runtime.rate_allowed(client):
        ...
        def _authorized(self) -> bool:
            ...
            if not supplied.startswith("Bearer "):
                return False
            return hmac.compare_digest(token, supplied[7:])
        ...
        def _require_auth(self) -> bool:
            ...
            self._json(
                ...
                {"ok": False, "error": "authentication_required", "scheme": "Bearer"},
            )
            return False
        ...
        def do_OPTIONS(self) -> None:
            ...
            if origin:
                self.send_header("Access-Control-Allow-Origin", origin)
                self.send_header("Vary", "Origin")
            self.send_header("Access-Control-Allow-Headers", "Authorization, Content-Type, X-GitHub-Event, X-GitHub-Delivery, X-Hub-Signature-256")
        ...
        def do_GET(self) -> None:
            ...
            if path in {"/", "/index.html"}:
                ...
                try:
                    ...
                    self._json(HTTPStatus.SERVICE_UNAVAILABLE, {"ok": False, "error": "command_center_unavailable", "message": str(error)})
                return
            ...
            if path == "/api/v1/state":
                self._json(HTTPStatus.OK, self.app.runtime.state_snapshot())
                return
            if path == "/api/v1/reports/latest":
                latest = self.app.runtime.last_result
                if latest is None:
                    self._json(HTTPStatus.NOT_FOUND, {"ok": False, "error": "no_report"})
                    return
                self._json(HTTPStatus.OK, latest)
                return
        ...
        def do_POST(self) -> None:
            ...
            try:
                ...
                if path == "/api/v1/settings":
                    ...
                    self._json(HTTPStatus.OK, {"ok": True, "settings": settings, "state": self.app.runtime.state_snapshot()})
                    return
                self._json(HTTPStatus.NOT_FOUND, {"ok": False, "error": "not_found"})
                ...
                self._json(HTTPStatus.UNPROCESSABLE_ENTITY, {"ok": False, "error": "invalid_request", "message": redact_secrets(error)})
...
def create_server(
    settings: StandaloneSettings,
    *,
    review_handler: ReviewHandler = handle_app_review_request,
...
def serve_forever(settings: StandaloneSettings) -> None:
    """Start the standalone service and block until interrupted."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--webhook-secret-env", default="SERGEANT_WEBHOOK_SECRET", help="Environment variable containing the GitHub webhook secret.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ...
        print(json.dumps({"ok": True, "configuration": settings.public_dict(), "capabilities": application.capabilities()}, indent=2, sort_keys=True))
        return 0
