...
def _tokens(value: object) -> set[str]:
    ...
    return {word for word in re.findall(r"[a-z0-9_]+", str(value or "").lower()) if len(word) > 2 and word not in stop}
...
def _matches(expected: list[dict[str, Any]], predictions: list[dict[str, Any]], threshold: float) -> tuple[list[FindingMatch], set[int]]:
    ...
    for item in expected:
        ...
        if matched:
            ...
            used.add(index)
        aliases = {str(item.get("category") or "").lower(), *[str(value).lower() for value in item.get("category_aliases", [])]}
        root = str(item.get("root_cause") or "")
        output.append(FindingMatch(
            expected_id=str(item.get("id") or "expected"),
            matched=matched,
            score=score,
            candidate=candidate,
            category_correct=bool(candidate and candidate.get("category") in aliases),
            severity_correct=bool(candidate and candidate.get("severity") == str(item.get("severity") or "").lower()),
            path_correct=bool(candidate and _path_ok(item, candidate)),
            line_correct=_line_ok(item, candidate) if candidate else (False if isinstance(item.get("line_start"), int) else None),
            root_cause_correct=(bool(candidate and candidate.get("root_cause") == root) if root else None),
        ))
    return output, used
...
def _completeness(predictions: list[dict[str, Any]]) -> float | None:
    ...
    for item in predictions:
        checks = [
            bool(item.get("message")), bool(item.get("evidence")),
...
def _mode_environment(mode: BenchmarkMode) -> Iterator[None]:
    names = {
        "SERGEANT_CPL_ENABLED", "SERGEANT_LLM_ENABLED", "SERGEANT_CPL_POLICY",
        "SERGEANT_LLM_POLICY", "SERGEANT_CPL_MAX_COUNCIL_MEMBERS", "SERGEANT_CPL_MAX_ROUNDS",
    }
    ...
    try:
        if mode == "deterministic":
            ...
            os.environ["SERGEANT_CPL_MAX_COUNCIL_MEMBERS"] = "1" if mode == "one-model" else os.getenv("SERGEANT_CPL_MAX_COUNCIL_MEMBERS", "5")
            os.environ["SERGEANT_CPL_MAX_ROUNDS"] = "1" if mode == "one-model" else os.getenv("SERGEANT_CPL_MAX_ROUNDS", "3")
        yield
    finally:
...
def run_case(payload: dict[str, Any], *, mode: BenchmarkMode, match_threshold: float = 0.55) -> BenchmarkCaseResult:
    ...
    tp = sum(item.matched for item in matches)
    fp = len(predictions) - len(used)
    fn = len(expected) - tp
    precision = tp / (tp + fp) if tp + fp else 1.0
    recall = tp / len(expected) if expected else (1.0 if not predictions else 0.0)
    ...
    return BenchmarkCaseResult(
        case_id=str(payload["id"]), title=str(payload.get("title") or payload["id"]), mode=mode,
        expected_verdict=expected_verdict, actual_verdict=actual, verdict_correct=actual == expected_verdict,
        expected_count=len(expected), prediction_count=len(predictions), true_positive_count=tp,
        false_positive_count=fp, false_negative_count=fn, precision=round(precision, 3),
        ...
        finding_completeness=_completeness(predictions), duration_ms=duration,
        cpl_status=str(cpl.get("status") or "missing"), model_call_count=len(passes), distinct_models=models,
        matches=matches,
...
def run_blind_benchmark(
    suite: str | Path, *, mode: BenchmarkMode = "deterministic", match_threshold: float = 0.55,
    minimum_precision: float = 0.6, minimum_recall: float = 0.75,
    require_route: bool = False, include_packets: bool = False,
) -> dict[str, Any]:
    ...
    tp = sum(item.true_positive_count for item in results)
    fp = sum(item.false_positive_count for item in results)
    precision = tp / (tp + fp) if tp + fp else 1.0
    recall = tp / expected_total if expected_total else (1.0 if fp == 0 else 0.0)
    f1 = 0.0 if precision + recall == 0 else 2 * precision * recall / (precision + recall)
    route_ready = mode == "deterministic" or all(item.cpl_status in {"completed", "completed_with_warnings"} for item in results)
    ...
    if require_route and not route_ready:
        passed = False
    return {
        "schema_version": RESULT_SCHEMA, "blind": True, "mode": mode, "case_count": len(results),
        "expected_finding_count": expected_total, "true_positive_count": tp, "false_positive_count": fp,
        ...
        "thresholds": {"match": match_threshold, "minimum_precision": minimum_precision, "minimum_recall": minimum_recall, "require_route": require_route},
        "passed": passed,
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = run_blind_benchmark(
        args.suite, mode=args.mode, match_threshold=args.match_threshold,
        minimum_precision=args.minimum_precision, minimum_recall=args.minimum_recall,
        require_route=args.require_route, include_packets=args.include_packets,
    )
