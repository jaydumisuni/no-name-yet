...
function summarizePayload(payload, exitCode) {
  const verdict = payload?.verdict?.verdict || payload?.review_verdict?.verdict || payload?.status || (exitCode === 0 ? "COMPLETED" : "FAILED");
  const status = payload?.status || payload?.verdict?.verdict || payload?.review_verdict?.verdict || verdict;
  const findingCount = payload?.evidence?.finding_count ?? payload?.verdict?.finding_count ?? payload?.review_verdict?.finding_count ?? collectFindings(payload).length;
  const action = payload?.action || payload?.verdict?.suggested_next_action || payload?.review_verdict?.suggested_next_action || "Review output";
  const normalized = String(verdict).toLowerCase();
  const cssClass = normalized.includes("pass") || normalized.includes("verified") || normalized.includes("completed") ? "pass" : normalized.includes("need") ? "needs" : normalized.includes("block") || normalized.includes("fail") ? "block" : "";
...
function renderFinding(finding) {
  ...
  return `<article class="finding ${escapeHtml(severity)}"><div class="finding-title">${escapeHtml(String(finding?.message || finding?.evidence || "Finding"))}</div>${pathText}<div class="muted">${escapeHtml([finding?.severity, finding?.category, finding?.provider].filter(Boolean).join(" / "))}</div></article>`;
}
